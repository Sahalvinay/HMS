...
df_patients = pd.read_csv(r"D:\Python projects\Hospital Management project\Patients_DataBase.csv")
df_doctors = pd.read_csv(r"D:\Python projects\Hospital Management project\Doctors_DataBase.csv")
df_appointments=pd.read_csv(r"D:\Python projects\Hospital Management project\appointments.csv")
tries = 0
tries_flag = ""
while tries_flag != "Close the program":
    ...
    if Admin_user_mode == "1":  # Admin mode
                ...
                if AdminOptions == "1":  # Admin mode --> Patients Management
                    ...
                    if Admin_choice == "1":  # Admin mode --> Patients Management --> Enter new patient data
                        ...
                        try:  # To avoid non integer input
                            ...
                            print_green("\npatient name        : "+df_patients.iloc[patient_ID, 1].capitalize())
                            print_green("patient age         : "+ str(df_patients.iloc[patient_ID, 2]))
                            print_green("patient gender      : "+ df_patients.iloc[patient_ID, 3])
                            print_green("patient address     : "+ df_patients.iloc[patient_ID, -1].capitalize())
                            if (df_patients.iloc[patient_ID, 1]) in list(df_appointments.iloc[:,1]):
                                ...
                                print_green("Department: "+df_appointments.iloc[count,-3].capitalize())
                                print_green("Doctor Following: "+df_appointments.iloc[count,2].capitalize())
                        ...
                        try:  # To avoid non integer input
                            ...
                            while True:
                                ...
                                Admin_choice = input("Enter your choice : ")
                                Admin_choice = Admin_choice.upper()
                                if Admin_choice == "1":#updating both patient and appointment database
                                    newName= input("\nEnter patient name : ").lower()
                    ...
                    if Admin_choice == "1":  # Admin mode --> Doctors Management --> Enter new doctor data
                        try:  # To avoid non integer input
                            ...
                            Department = input("Enter Doctor department : ").lower()
                            Name = input("Enter Doctor name       : ").lower()
                            Address = input("Enter Doctor address    : ").lower()
                            df_doctors.loc[len(df_doctors.index)]=[Doctor_ID+1,Department,Name,Address]
                        ...
                        try:  # To avoid non integer input
                            ...
                            while Doctor_ID not in df_doctors.index:
                                ...
                                print_green("\nDoctor name    : "+ df_doctors.iloc[Doctor_ID, 2].capitalize())
                                print_green("Doctor address : "+ df_doctors.iloc[Doctor_ID, 3].capitalize())
                                print_green("Doctor is in " + df_doctors.iloc[Doctor_ID, 1].capitalize() + " department")
                        except:
                            print_red("Doctor ID should be an integer number")
                        ...
                        try:  # To avoid non integer input
                            ...
                            Admin_choice = Admin_choice.upper()
                            if Admin_choice == "1":
                                df_doctors.iloc[Doctor_ID, 1] = input("Enter Doctor's Department : ").lower()
                                print_green(
                                    "/----------------------Doctor's department edited successfully----------------------/")

                            elif Admin_choice == "2":
                                df_doctors.iloc[Doctor_ID, 2] = input("Enter Doctor's Name : ").lower()
                                print_green(
                                    "----------------------Doctor's name edited successfully----------------------")

                            elif Admin_choice == "3":
                                df_doctors.iloc[Doctor_ID, -1] = input("Enter Doctor's Address : ").lower()
                                print_green(
                    ...
                    print("|To check for  an appointment Enter 4       |")
                    print("|To be back enter E                     |")
                    print("-----------------------------------------")
                    Admin_choice = input("Enter your choice : ")
                    Admin_choice = Admin_choice.upper()
                    if Admin_choice == "1":  # Admin mode --> Appointment Management --> Book an appointment
                        try:  # To avoid non integer input
                            ...
                            if Admin_choice == "1":
                                ...
                                Department = input("Enter the department to which admission must be done: ").lower()
                                ...
                                df_appointments.loc[len(df_appointments.index)]=[uniqueID,df_patients.iloc[patient_ID,1].lower(),df_doctors.iloc[doctorIndex,-2].lower(),Department.lower(),roomNumber,random.choice(['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'])]
                                ...
                                patient_appnt = input("Which department you want to take admission on?").lower()
                                if patient_appnt not in list(df_doctors.iloc[:, 1]):
                                    ...
                                    Age = input("Enter patient age     : ").lower()
                                    Gender = input("Enter patient gender  : ").lower()
                                    Address = input("Enter patient address : ").lower()
                                    RoomNumber = ""
                                ...
                                df_appointments.loc[len(df_appointments.index)]=[uniqueID,df_patients.iloc[patient_ID,1].lower(),df_doctors.iloc[doctorIndex,-2].lower(),patient_appnt.lower(),roomNumber,random.choice(['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'])]
                        ...
                        try:  # To avoid non integer input
                            ...
                            if patientUniqueID in list(df_appointments.iloc[:,0]):
                                ...
                                if adminOption==1:#update both appointment and patient database
                                    ...
                                    newDept=input("Enter department:").lower()
                        ...
                        try:  # To avoid non integer input
                            ...
                            while unique_ID not in list(df_appointments.iloc[:,0]):
                                unique_ID = int(input("Incorrect ID, Enter patient ID : "))
                            try:
                                posName=list(df_appointments.iloc[:,0]).index(unique_ID)
                        ...
                        try:
                            uniqueID=int(input(("Enter Unique ID:")))
                            while uniqueID not in list(df_appointments.iloc[:,0]):
                                print("Unique ID not present!")
                                break
                            positionAppointment=list(df_appointments.iloc[:,0]).index(uniqueID)
                            print_green("Patient Name: "+df_appointments.iloc[positionAppointment,1])
                            print_green("Doctor Name: "+df_appointments.iloc[positionAppointment,2])
                            print_green("Department: "+df_appointments.iloc[positionAppointment,3])
                            print_green("Room Number: "+str(df_appointments.iloc[positionAppointment,4]))
                            print_green("Appointment Day: "+str(df_appointments.iloc[positionAppointment,-1]))
                        except ValueError:
                            print_red("  Non Integer Input!!")
                    elif Admin_choice == "E":  # Back
                        break
                    ...
                    if checkDataSave.lower()=='y':
                        df_patients.to_csv(r"D:\Python projects\Hospital Management project\Patients_DataBase.csv",index=False)
                        df_appointments.to_csv(r"D:\Python projects\Hospital Management project\appointments.csv",index=False)
                        df_doctors.to_csv(r"D:\Python projects\Hospital Management project\Doctors_DataBase.csv",index=False)
                    break
                else:
        ...
        while True:
            ...
            print("|To view appointments Enter 3     |")
            print("|To be Back Enter E                     |")
            print("-----------------------------------------")
            ...
            if UserOptions == "1":  # User mode --> view hospital's departments
                ...
                try:  # To avoid non integer input
                    unique_ID = int(input("Enter Unique ID : "))
                    while unique_ID not in list(df_appointments.iloc[:,0]):
                        unique_ID = int(input("Incorrect Id, Please enter correct Unique ID : ")) - 1
                    positionAppointment=list(df_appointments.iloc[:,0]).index(unique_ID)
                    print_green("Patient name        : "+ df_appointments.iloc[positionAppointment,1].capitalize())
                    print_green("Doctor name         : "+ df_appointments.iloc[positionAppointment,2].capitalize())
                    print_green("Department          : "+ df_appointments.iloc[positionAppointment,3].capitalize())
                    print_green("Room Number         : "+ str(df_appointments.iloc[positionAppointment,-2]))
                except ValueError:
                    print("Unique ID should be an integer number")
